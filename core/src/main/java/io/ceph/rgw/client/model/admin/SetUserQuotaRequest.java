 ...
 */
public class SetUserQuotaRequest extends AdminRequest {
    private static final SdkField<String> UID_FIELD = SdkField
            .builder(MarshallingType.STRING)
            .getter(getter(SetUserQuotaRequest::getUid))
    ...
    private static final SdkField<Quota> QUOTA_FIELD = SdkField
            ...
            .getter(getter(SetUserQuotaRequest::getQuota))
    ...
    public SetUserQuotaRequest(Builder builder) {
        super(builder);
        this.uid = Validate.notBlank(builder.uid, "uid cannot be empty string");
        this.quota = Objects.requireNonNull(builder.quota);
    }

    private static <T> Function<Object, T> getter(Function<SetUserQuotaRequest, T> g) {
        return obj -> g.apply((SetUserQuotaRequest) obj);
    }
    ...
    public static class Builder extends AdminRequestBuilder<Builder, SetUserQuotaRequest, SetUserQuotaResponse> {
        private String uid;
        private Quota quota;
        ...
        private Builder(SetUserQuotaRequest request) {
            this.quota = new Quota();
            withUid(request.uid);
        ...
        @Override
        public SetUserQuotaRequest build() {
            return new SetUserQuotaRequest(this);
        }

        @Override
        public SetUserQuotaResponse run() {
            return client.setUserQuota(build());
        }

        @Override
        public ActionFuture<SetUserQuotaResponse> execute() {
            return client.setUserQuotaAsync(build());
        }

        @Override
        public void execute(ActionListener<SetUserQuotaResponse> listener) {
            client.setUserQuotaAsync(build(), listener);
        }
    }
}
